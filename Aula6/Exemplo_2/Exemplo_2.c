...
Estrutura_Pilha pilha [TAMANHO]; //variaveis globais

void Push_Pilha(int valor1, int valor2, int valor3)
{
    if (topo < TAMANHO){
        pilha[topo].data = valor1;
        pilha[topo].idade = valor2;
        pilha[topo].matricula = valor3;
        topo++;
        printf("\nInsercao completo\n");
    }
    else
...
void List_Pilha()
{
    ...
    for (i = topo-1; i > -1; i--)
    {
        printf("A idade na posicao %i e : %i\n", i+1, pilha[i].idade);
        printf("A data na posicao %i e : %i\n", i+1, pilha[i].data);
        printf("O matricula na posicao %i e : %i\n", i+1, pilha[i].matricula);
    }
}
